...
However, we could also load a packmonths_as_customer and use a function to do it quickly. For example, the effectsize packmonths_as_customer has a standardize() function.

# install packmonths_as_customer if needed
```{r}
install.packages("effectsize")
import(effectssize)
autinsurance$z_of_months_as_customer <- effectsize::standardize(autinsurance$months_as_customer)
```
...
```{r}
write.csv(autinsurance,'autinsurance_V3.csv')
```
